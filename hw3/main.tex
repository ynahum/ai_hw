...
\begin{enumerate}
  \item Start with any initial value function $V_0(s),\quad \forall s\in S$.
  ...
  \item Apply a stopping rule to obtain a required accuracy.\\
...
V^{*}||_\infty \leq \epsilon$
   ...
   \[\pi(s) = arg\max_{a\in A(s)} \sum_{s' \in S} P(s'|s,a)[R(s,a,s') + \gamma V^*(s')], \quad \forall s \in S\]
\end{enumerate}

For a fixed policy value iteration, the recursive computation is changed to have $\pi(s)$ instead of $a$
$$V^{\pi}_{n + 1}(s) = \sum_{s' \in S} P(s'|s,a)[R(s,a,s') + \gamma V^{\pi}_n(s')], \quad \forall s \in S$$
